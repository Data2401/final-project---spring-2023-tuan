...
- There is a limitation in term of data information about the observations of `location` variable in the raw data. Most of the Nightlife observations like Bar/Club/Restaurants was classified as `Other`, which make the original data having a large quantity of observations in `Other` category.
...
```{r General Plot, echo=FALSE, message=FALSE, warning=FALSE}
library(plotly)
...
hp <- my_data %>% 
  ...
  summarize(Cases = n()) %>% 
  arrange(desc(.)) %>% 
  mutate(Percentage = 100 * Cases/sum(Cases)) %>% ggplot(aes(x=year, y = Cases)) +
  geom_histogram(stat = 'identity', fill = 'yellow') +
  theme_dark() +
  labs(title = 'Incidents over Year',
       x = 'Year',
       y = 'Cases') 
ggplotly(hp)
```
...
my_data %>%
  ...
  select(Case = case, Victims = total_victims, Fatality =  fatalities, Injured = injured, City = city, State = state, Date = date, Age = age_of_shooter, Race = race, Gender = gender, Weapons = weapon_type)
```
...
- They are light weight, high bullet rate and big magazines. It could help the shooters cause mass casualties in a short period of time.
...
p <- my_data %>%
  ...
  geom_point() + labs(title = 'Age of The Shooters over Time',
                        x = 'Year',
...
```
At the first glance, we are not able to say anything about the relationships between the ages of the shooters over the time. Hence I decide to divide the graph into the zone and sub-areas like the graph below. 
...
+ Zone II is the area where the incident occurs the most. Since this age’s range is the main force in the labor market, which explains why the mass shooting cases at the workplace appear to be the most.
...
pp + geom_point() + labs(title = 'Age of The Shooters over Time',
                        x = 'Year',
                        y = 'Age of the Shooters',
                        color ='Gender',
                        shape = 'Prior Signs of Mental Health Issues') +
...
map_plot <-  ggplot(states_map, aes(long, lat)) +
  ...
  geom_point(data = my_data_notHInAL, 
             aes(x = longitude, y = latitude, 
                 size = total_victims, color = fatalities)) +
